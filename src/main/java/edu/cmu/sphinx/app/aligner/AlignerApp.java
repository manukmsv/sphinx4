 ...
 *
 */
package edu.cmu.sphinx.app.aligner;
...
public class AlignerApp {
    ...
    public static void main(String args[]) throws Exception {
        ...
        if (args.length > 1) {
            ...
            audioUrl = AlignerApp.class.getResource("10001-90210-01803.wav");
            transcript = TEXT;
        }
        ...
        for (int i = 0; i < aid.length; ++i) {
            if (aid[i] == -1) {
                System.out.format("- %s", words.get(i));
            } else {
                if (aid[i] - lastId > 1) {
                    for (WordResult result : results.subList(lastId + 1,
                            aid[i])) {
                        System.out.format("+ %s", result.getWord()
                                .getSpelling());
                    }
                }
                System.out.format("  %s", results.get(aid[i])
                        .getWord().getSpelling());
                lastId = aid[i];
            }
        }
